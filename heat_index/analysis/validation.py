...
from heat_index.analysis.pixels import SAMPLE_FILES, load_config, project_path, write_csv, write_markdown
...
def validate_supporting_outputs(config: dict[str, Any], checks: Checks) -> None:
    ...
    if str(config.get("analysis_scope", "")).startswith("schema-0.2 canonical"):
        ...
        spatial_method = PROJECT_ROOT / "heat_index" / "visualization" / "spatial_maps.py"
