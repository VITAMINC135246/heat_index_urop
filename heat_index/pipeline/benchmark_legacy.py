...
from heat_index.io.validation import GroupInput, validate_group
from heat_index.classification.polygon import rasterize_polygon
...
def main() -> int:
    ...
    pairs = pd.read_excel(data_root(PROJECT_ROOT) / "metadata" / "part_b_pilot_pairs.xlsx").head(args.max_pilot_groups)
    part_d = pd.read_csv(
    ...
    for _, pair in pairs.iterrows():
        group = GroupInput(
            ...
            visible_path=str(resolve_path(str(pair["v_path"]), PROJECT_ROOT)),
            thermal_path=str(resolve_path(str(pair["t_path"]), PROJECT_ROOT)),
            dataset_id="pilot",
        )
        ...
        if len(rows) == 1:
            matrix_path = resolve_path(str(rows.iloc[0]["npy_path"]), PROJECT_ROOT)
    ...
    canonical_path = data_root(PROJECT_ROOT) / "processed" / "part_e" / "part_e_pixel_delta_t.parquet"
    ...
    pixel_csvs = list((data_root(PROJECT_ROOT) / "processed" / "part_e" / "excel_source").glob("*_pixel_data.csv"))
    pixel_workbooks = list((PROJECT_ROOT / "outputs" / "part_e" / "excel" / "pixel_workbooks").glob("*.xlsx"))
    canonical_directories = [
        path for path in (data_root(PROJECT_ROOT) / "processed" / "images").glob("*")
