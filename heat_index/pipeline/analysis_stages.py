...
from heat_index.analysis.pixels import SAMPLE_FILES, load_config, project_path
...
def py(script: str, args: argparse.Namespace, *extra: str) -> Command:
    modules = {
        "00_audit_part_e_inputs.py": "heat_index.analysis.input_audit",
        "01_build_pixel_delta_t_dataset.py": "heat_index.analysis.pixel_dataset",
        "02_build_pixel_analysis_samples.py": "heat_index.analysis.sampling",
        "03_run_pixel_statistical_analysis.py": "heat_index.analysis.statistics",
        "04_generate_pixel_delta_t_spectra.py": "heat_index.visualization.spectra",
        "05_generate_pixel_spatial_figures.py": "heat_index.visualization.spatial_maps",
        "06_build_per_image_pixel_workbooks.py": "heat_index.reporting.pixel_workbooks",
        "07_build_main_excel_workbook.py": "heat_index.reporting.summary_workbook",
        "09_export_excel_charts.py": "heat_index.reporting.excel_charts",
        "10_validate_part_e_outputs.py": "heat_index.analysis.validation",
        "11_generate_part_e_report.py": "heat_index.reporting.analysis_report",
    }
    return Command([sys.executable, "-m", modules[script], "--config", args.config, *extra])
...
def stage_definitions() -> list[Stage]:
    ...
    def spatial_outputs(args: argparse.Namespace, config: dict) -> list[Path]:
        ...
        try:
            ...
            method_path = PROJECT_ROOT / "heat_index" / "visualization" / "spatial_maps.py"
