...
from heat_index.config.paths import resolve_path
...
from heat_index.io.validation import discover_dataset_groups  # noqa: E402

PILOT_TABLE = data_root(PROJECT_ROOT) / "metadata" / "part_b_pilot_pairs.xlsx"


def resolve(value: str | Path) -> Path:
    path = Path(value)
    return resolve_path(path, PROJECT_ROOT).resolve()
...
def interactive_session() -> argparse.Namespace:
    ...
    if workflow in {"soccer", "all"}:
        base = data_root(PROJECT_ROOT) / "raw" / "HKUST" / "20260202_Thermal_HKUST" / "DCIM" / "DJI_202602020853_001"
        common.update(
            {
...
def build_command(args: argparse.Namespace) -> list[str]:
    ...
    command = [
        workflow_python(),
        "-m", "heat_index.pipeline.workflow",
        "--config",
        config,
        "--require-all-success",
        "--interactive",
