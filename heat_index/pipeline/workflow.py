...
from heat_index.config.paths import data_root, resolve_path
...
from heat_index.io.canonical import write_canonical_result
from heat_index.provenance.capture_time import (
    CAPTURE_TIME_RESOLVER_VERSION,
    DJI_METADATA_PATH,
)
from heat_index.analysis.extremes import extreme_summary, plot_pixel_extremes
from heat_index.io.validation import (
    discover_dataset_groups,
    image_identifier,
    validate_group,
    write_validation_record,
)
from heat_index.spatial.landuse import (
    LUHK_CATEGORIES,
    LUHK_LOOKUP_VERSION,
    load_native_luhk_result,
    resolve_luhk_context,
)
from heat_index.provenance.models import (
    ArtifactReference,
    ContentTriageState,
    CoverageClass,
    GroupInput,
...
from heat_index.spatial.correspondence import run_full_part_b
from heat_index.spatial.triage import (
    apply_part_b0_review,
    load_part_b0_reviews,
    triage_content_correspondence,
    write_part_b0_result,
)
from heat_index.spatial.review import load_review_decisions, resolve_decision, verified_pilot_decisions
from heat_index.classification.surface_cover import load_reviewed_label_override, run_reviewed_part_c
from heat_index.analysis.aggregate import aggregate_canonical_results, write_run_summary
from heat_index.pipeline.analysis import run_formal_part_e
from heat_index.io.pilot import adapt_pilot_image, pilot_image_ids, pilot_source_paths
from heat_index.classification.polygon import polygon_context_arrays, run_polygon_gui_subprocess
from heat_index.provenance.result_index import ResultIndex, configuration_hash, sha256_file, source_hashes
from heat_index.pipeline.routing import finalize_part_b, route_after_part_a, route_after_part_b0, route_group
...
from heat_index.thermal.artifact import sidecar_path, write_thermal_artifact
from heat_index.analysis.temporal import TemporalAnalysisPlan, parse_temporal_plan
from heat_index.visualization.temporal_review import collect_temporal_plan
...
def cover_mapping() -> tuple[dict[int, str], dict[str, int], Path]:
    path = data_root(PROJECT_ROOT) / "annotations" / "part_c" / "surface_cover_class_mapping.xlsx"
...
def legacy_temperature(image_id: str) -> TemperatureResult | None:
    ...
    if not path.is_file():
        return None
    return load_legacy_temperature(path, row, image_id=image_id, source_image_path=project_path(str(row.get("t_path", ""))))
    ...
    *,
    ...
    if image_id in overrides:
        return load_temperature_override(
            ...
            image_id=image_id,
            source_image_path=thermal_path,
        )
    # An explicit TAT3 input requests extraction with those exact parameters;
    # a previous pilot matrix must not shadow that request.
    if not tat3_report_paths(args) and not args.tat3_params_csv:
    ...
    result = extract_temperature(
        ...
        work_directory=data_root(PROJECT_ROOT) / "processed" / "part_d" / "temperature_matrices" / "sdk_raw",
        irp_exe=irp,
    ...
    if artifact_path is not None:
        write_thermal_artifact(artifact_path, result, image_id=image_id)
    return result
...
def main() -> int:
    ...
    for group_number, group in enumerate(groups, start=1):
        ...
        capture_time_dependency_paths = [
            DJI_METADATA_PATH,
            PROJECT_ROOT / "heat_index" / "provenance" / "capture_time.py",
        ]
        source_paths.extend(path for path in capture_time_dependency_paths if path.is_file())
        luhk_dependency_paths = [
            data_root(PROJECT_ROOT) / "processed" / "grids" / "pilot_luhk_aligned_10m_grid_cells.xlsx",
            data_root(PROJECT_ROOT) / "processed" / "footprints" / "image_footprints.xlsx",
            data_root(PROJECT_ROOT) / "luhk" / "LUMHK_RasterGrid_2024.tif",
            PROJECT_ROOT / "heat_index" / "spatial" / "landuse.py",
        ]
        ...
        if image_id in temperature_overrides:
            ...
            override_sidecar = sidecar_path(temperature_overrides[image_id])
            if override_sidecar.is_file():
                source_paths.append(override_sidecar)
        ...
        try:
            ...
            if route.route == ProcessingRoute.NORMAL_VT and image_id in pilot_ids:
                ...
                temperature = temperature_for_group(
                    ...
                    args=args,
                    artifact_path=group_dir / "thermal" / f"{image_id}_temperature_celsius.npy",
                )
