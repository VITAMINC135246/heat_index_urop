...
from heat_index.config.paths import data_root, logical_path

from collections import Counter
from pathlib import Path

from heat_index.provenance.camera_profiles import classify_matrice_4t_camera
from heat_index.io.tables import read_table, write_rows


PROJECT_ROOT = Path(__file__).resolve().parents[2]
METADATA_XLSX = data_root(PROJECT_ROOT) / "metadata" / "dji_image_metadata.xlsx"
OUTPUT_XLSX = data_root(PROJECT_ROOT) / "metadata" / "camera_metadata_validation_summary.xlsx"
...
def main() -> int:
    ...
    print(f"Wrote {logical_path(OUTPUT_XLSX, PROJECT_ROOT).as_posix()}")
    return 0
