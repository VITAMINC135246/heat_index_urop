...
from heat_index.provenance.camera_profiles import resolve_camera_parameters
from heat_index.io.tables import read_table, write_table
    ...
    row.update(
        {
            ...
            "metadata_source": relative_posix(resolve_path(METADATA_XLSX_NAME, project_root())),
...
def write_summary(rows: list[dict[str, Any]], summary_txt: Path) -> None:
    ...
    lines = [
        ...
        "",
        f"output XLSX path: {relative_posix(resolve_path(OUTPUT_XLSX_NAME, project_root()))}",
...
def main() -> int:
    ...
    root = project_root()
    try:
        vt_pairs_df, metadata_df = load_inputs(resolve_path(VT_PAIRS_XLSX_NAME, root), resolve_path(METADATA_XLSX_NAME, root))
    ...
    output_xlsx = resolve_path(OUTPUT_XLSX_NAME, root)
    ...
    thermal_rows = [row for row in rows if row.get("image_type") == "thermal"]
    write_xlsx_rows(thermal_rows, resolve_path(THERMAL_COMPAT_XLSX_NAME, root))
