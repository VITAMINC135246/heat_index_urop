...
def find_luhk_raster() -> Path:
    root = project_root()
    extract_dir = resolve_path(LUHK_EXTRACT_DIR_NAME, root)
    ...
    for path in candidates:
        if any(token in path.name.casefold() for token in ("blu", "luhk", "lumhk")):
            return path
    zip_candidates = sorted((data_root(root)).glob("*LUHK*GEOTIFF*.zip"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    root = project_root()
    footprints_xlsx = resolve_path(IMAGE_FOOTPRINTS_XLSX_NAME, root)
    ...
    output_xlsx = resolve_path(OUTPUT_GRID_XLSX_NAME, root)
    ...
    write_xlsx(all_rows, resolve_path(COMPAT_GRID_XLSX_NAME, root))
