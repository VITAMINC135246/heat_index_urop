...
from heat_index.visualization.backend import interactive_pyplot as _interactive_pyplot
...
class SuperpixelReviewController:
    ...
    def generate_masks(self) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        from heat_index.classification import reviewed_masks as legacy
...
def run_review_gui_subprocess(controller: SuperpixelReviewController, draft_path: Path, segment_path: Path) -> str:
    ...
    completed = subprocess.run(
        [sys.executable, "-m", "heat_index.classification.review", "--session", str(session_path)],
