...
from heat_index.io.canonical import UNKNOWN_LABEL_ID, write_canonical_result
from heat_index.provenance.capture_time import dji_metadata_for_paths, resolve_capture_time
from heat_index.spatial.landuse import LUHK_CATEGORIES, LUHK_LOOKUP_VERSION, load_native_luhk_result
from heat_index.provenance.models import CoverageClass, ManualReviewStatus, ProcessingRoute, QAStatus, SceneCorrespondence, SourceMethod
from heat_index.provenance.result_index import configuration_hash, source_hashes
from heat_index.thermal.artifact import sidecar_path
from heat_index.thermal.extraction import TemperatureResult, load_legacy_temperature
...
def pilot_source_paths(project_root: Path, image_id: str) -> list[Path]:
    ...
    pairs_path = data_root(project_root) / "metadata" / "part_b_pilot_pairs.xlsx"
    ...
    thermal_sidecar = sidecar_path(_project_path(project_root, temperature_row["npy_path"]))
    if thermal_sidecar.is_file():
        paths.append(thermal_sidecar)
    official_raster = data_root(project_root) / "luhk" / "LUMHK_RasterGrid_2024.tif"
...
def adapt_pilot_image(
    ...
    *,
    ...
    mapping = pd.read_excel(data_root(project_root) / "annotations" / "part_c" / "surface_cover_class_mapping.xlsx")
    pair_rows = pairs.loc[pairs["image_id"].astype(str).eq(image_id)]
    mask_rows = masks.loc[masks["image_id"].astype(str).eq(image_id)]
    temperature_rows = temperatures.loc[temperatures["image_id"].astype(str).eq(image_id)]
    if len(pair_rows) != 1 or len(mask_rows) != 1 or len(temperature_rows) != 1:
    ...
    if temperature_result is None:
        accepted = load_legacy_temperature(
            _project_path(project_root, temperature_row["npy_path"]),
            temperature_row,
            image_id=image_id,
            source_image_path=_project_path(project_root, temperature_row.get("t_path", "")),
        )
        temperature = accepted.matrix
        runtime_temperature_metadata = dict(accepted.metadata)
        runtime_qa = (
            QAStatus.WARN
    ...
    return write_canonical_result(
        output_root=output_root or data_root(project_root) / "processed" / "images",
        image_id=image_id,
        ...
        ambient_metadata={
            ...
            "source": runtime_temperature_metadata.get("ambient_source", runtime_temperature_metadata.get("source", "unknown")),
            "definition": runtime_temperature_metadata.get("ambient_definition", runtime_temperature_metadata.get("definition", "unknown")),
            "source_record": runtime_temperature_metadata.get("ambient_source_record", runtime_temperature_metadata.get("source_record", "")),
            "validation_status": runtime_temperature_metadata.get("ambient_validation_status", runtime_temperature_metadata.get("validation_status", "unknown")),
        },
