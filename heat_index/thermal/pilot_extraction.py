...
from heat_index.config.paths import logical_path, resolve_path

from heat_index.config.paths import data_root
...
from heat_index.io.tables import write_rows
from heat_index.thermal.extraction import extract_temperature, temperature_qa

PILOT_PAIRS_XLSX = data_root(PROJECT_ROOT) / "metadata" / "part_b_pilot_pairs.xlsx"
DJI_METADATA_XLSX = data_root(PROJECT_ROOT) / "metadata" / "dji_image_metadata.xlsx"
CLASS_MAPPING_XLSX = data_root(PROJECT_ROOT) / "annotations" / "part_c" / "surface_cover_class_mapping.xlsx"
...
TEMPERATURE_DIR = data_root(PROJECT_ROOT) / "processed" / "part_d" / "temperature_matrices"
...
def relative_posix(path: Path | str) -> str:
    return logical_path(path, PROJECT_ROOT).as_posix()
...
def find_exiftool() -> str:
    candidates = [
        os.environ.get("HEAT_INDEX_EXIFTOOL", "").strip(),
        "exiftool",
        "exiftool.exe",
...
def update_doc(tool: SdkTool, exiftool_status: str, ran_rows: list[dict[str, Any]]) -> None:
    ...
    lines = [
        ...
        "## Dependency Rule",
        "",
        "Keep DJI Thermal SDK binaries outside the repository and point to their location with an ignored local config file.",
        "",
        "Tracked template:",
        "",
        "```powershell",
...
def main() -> int:
    args = parse_args()
    if os.name != "nt":
        raise RuntimeError("DJI Thermal SDK extraction requires Windows with dji_irp.exe; use a validated cached thermal artifact for Mac analysis.")
