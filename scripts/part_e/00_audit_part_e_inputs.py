...
def main() -> int:
    ...
    for _, record in records.iterrows():
        ...
        class_mask, shadow_mask = load_masks(record)
        ...
        if matrix.shape != EXPECTED_SHAPE:
            problems.append(f"{image_id}: temperature shape mismatch")
        ...
        inventory.append(
            {
                ...
                "orientation_status": "structurally_aligned_shared_512x640_thermal_grid",
                "garden_hill_excluded": True,
            }
        )
        orientation_notes.append(
            f"- `{image_id}`: temperature, cover, and shadow arrays share `(512, 640)` row-major thermal-grid shape; "
    ...
    lines = [
        ...
        f"- Temperature matrices at expected shape: **{int(inventory_frame['temperature_shape'].eq('512x640').sum())} / {len(records)}**.",
        f"- Ambient mapping: **one finite TAT3 record per image**.",
        f"- Structural cover/shadow alignment: **{int(inventory_frame['surface_cover_shape'].eq('512x640').sum())} / {len(records)}** cover masks and **{int(inventory_frame['shadow_shape'].eq('512x640').sum())} / {len(records)}** shadow masks.",
        ...
        *orientation_notes,
        ...
        "## Shadow audit",
        "",
        "All five reviewed shadow masks exist and are aligned, but every pixel is flagged `0` (no shadow recorded). "
        "The formal dataset therefore marks shadow as known while reporting that no shadow-present comparison is estimable. Missing masks would remain null and invalid, never silently converted to zero.",
        "",
