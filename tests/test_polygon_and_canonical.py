...
class PolygonCanonicalTests(unittest.TestCase):
    ...
    def test_dynamic_dimensions_and_optional_shadow(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertTrue(pixels.loc[~pixels["label_known"], "exclusion_reason"].eq("outside_target_selection").all())
            self.assertFalse(pixels.loc[~pixels["label_known"], "analysis_eligible"].any())
            self.assertEqual(loaded.reviewer_confidence, "high")
            self.assertEqual(loaded.notes, "target boundary checked")
            self.assertTrue((Path(directory) / "synthetic_T" / "target_mask.npy").is_file())
    ...
    def test_polygon_requires_luhk_dimension_match_and_preserves_provenance_kind(self) -> None:
        user = resolve_luhk_context("GIC / open space", "user_supplied_luhk")
        official = resolve_luhk_context("gic_open_space", "official_luhk_lookup")
        self.assertNotEqual(user.provenance, official.provenance)
        shape = (3, 3)
        labels, known = labelled_polygon_arrays([(0, 0), (2, 0), (2, 2)], shape, 1)
        with tempfile.TemporaryDirectory() as directory:
            common = dict(
                output_root=Path(directory), image_id="i", group_id="g", pair_id="p", dataset_id="d",
                visible_path="", thermal_path="t", temperature=np.ones(shape),
                processing_route=ProcessingRoute.THERMAL_POLYGON,
                source_method=SourceMethod.THERMAL_POLYGON_USER_ANNOTATION,
                review_status=ManualReviewStatus.ACCEPTED, qa_status=QAStatus.PASS,
                configuration_hash="c", source_file_hashes={}, surface_cover_class_id=1,
                surface_cover_category="roof", polygon_coordinates=[[0, 0], [2, 0], [2, 2]],
            )
            with self.assertRaisesRegex(ValueError, "LUHK"):
                write_canonical_result(labels=labels, known_mask=known, **common)
            with self.assertRaisesRegex(ValueError, "shapes must match"):
                write_canonical_result(
                    labels=np.ones((2, 2), dtype=np.int16), known_mask=np.ones((2, 2), dtype=bool),
                    luhk_category="GIC / open space", luhk_provenance="user_supplied_luhk", **common,
                )
